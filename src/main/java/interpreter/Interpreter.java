...
public class Interpreter {
     public static List<IStmt> getPrgStates() throws MyException {
         ...
         IStmt ex13 = new CompStmt(new AddProcStmt("sum", new ArrayList<>(Arrays.asList("a", "b")),
                 new CompStmt( new VarDeclStmt("v", new IntType()),
                         new CompStmt(new AssignStmt("v", new ArithExp(new VarExp("a"), new VarExp("b"), 1)),
                                 new PrintStmt(new VarExp("v"))))),
                 new CompStmt(new AddProcStmt("product", new ArrayList<>(Arrays.asList("a", "b" )),
                         new CompStmt(new CompStmt(new VarDeclStmt("v", new IntType()), new AssignStmt("v", new ArithExp(new VarExp("a"), new VarExp("b"), 3))),
                                 new PrintStmt(new VarExp("v")))),
                         new CompStmt(new VarDeclStmt("v", new IntType()),
                                 new CompStmt(new VarDeclStmt("w", new IntType()),
                                         new CompStmt(new AssignStmt("v", new ValueExp(new IntValue(2))),
                                                 new CompStmt(new AssignStmt("w", new ValueExp(new IntValue(5))),
                                                         new CompStmt(new CallStmt("sum",   new ArithExp(new VarExp("v"), new ValueExp(new IntValue(10)), 3), new VarExp("w")),
                                                                 new CompStmt(new PrintStmt(new VarExp("v")),
                                                                         new CompStmt(new ForkStmt( new CallStmt("product",   new VarExp("v"), new VarExp("w")) ),
                                                                                 new ForkStmt(new CallStmt( "sum", new VarExp("v"), new VarExp("w") ))))) ))))));

         return new ArrayList<>(Arrays.asList(ex1,ex2,ex3,fin,ex5,ex6,ex7,ex8,ex9,ex10,ex11,ex12, ex13));
     }
}
