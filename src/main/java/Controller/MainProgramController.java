...
public class MainProgramController implements Initializable {
    ...
    @FXML
    TableView<LatchTableView> LatchTable;

    @FXML
    TableColumn<LatchTableView, Integer> LocationLatchColumn;

    @FXML
    TableColumn<LatchTableView, Integer> ValueLatchColumn;
    ...
    public void populateLatchTable() throws MyDictionaryException {
        if(programController.getRepo().getPrgList().isEmpty())
            return;
        LatchTable.getItems().clear();
        int getIndex = selected;
        if(getIndex < 0)
            getIndex = 0;
        List<PrgState> states = programController.getRepo().getPrgList();

        int programID = states.get(getIndex).getId();

        MyITable<Integer, Integer> sym = programController.getRepo().getById(programID).getLatchTable();
        Enumeration<Integer> e = sym.keys();
        while(e.hasMoreElements())
        {
            Integer a = e.nextElement();
            LatchTable.getItems().add(new LatchTableView(a, sym.lookup(a)));
        }
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        LocationLatchColumn.setCellValueFactory(new PropertyValueFactory<LatchTableView, Integer>("loc"));
        ValueLatchColumn.setCellValueFactory(new PropertyValueFactory<LatchTableView, Integer>("val"));
    ...
    public void loadProgram(IStmt statement, int index) throws MyException, IOException {
        ...
        MyITable<Integer, Integer> latch = new MyTable<>();
        ...
        PrgState program = new PrgState(executionStack, symbolTable, output, fileTable, heap1, latch);
