...
public class MainProgramController implements Initializable {
    ...
    @FXML
            TableView<ProcTableValue> ProcedureTable;
    @FXML
            TableColumn<ProcTableValue, String> ProcedureColumn;
    @FXML
            TableColumn<ProcTableValue, String> BodyColumn;
    int selected = -1;
    ...
    public void loadProgram(IStmt statement, int index) throws MyException, IOException {
        ...
        MyITable<String, Pair<ArrayList<String>, IStmt>> procTable = new MyTable<>();
        ...
        PrgState program = new PrgState(executionStack, symbolTable, output, fileTable, heap1, procTable);
    ...
    private void populateProcedureTable() throws MyDictionaryException {
        if(programController.getRepo().getPrgList().isEmpty())
            return;
        ...
        MyITable<String, Pair<ArrayList<String>, IStmt>> sym = programController.getRepo().getById(programID).getProcTable();
        Enumeration<String> e = sym.keys();
        while(e.hasMoreElements())
        {
            String a = e.nextElement();
            ProcedureTable.getItems().add(new ProcTableValue(a + " " + sym.lookup(a).getKey(), sym.lookup(a).getValue().toString()));
        }
    }
