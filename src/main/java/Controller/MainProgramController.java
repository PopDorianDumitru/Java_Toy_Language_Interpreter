...
public class MainProgramController implements Initializable {
    ...
    @FXML
    TableView<SemTableValue> SemaphoreTableView;

    @FXML
            TableColumn<SemTableValue, Integer> ValueSemaphoreColumn;
    @FXML
            TableColumn<SemTableValue, Integer> IndexSemaphoreColumn;
    @FXML
            TableColumn<SemTableValue, ArrayList<Integer>> ListSemaphoreColumn;
    ...
    private void populateSemaphoreTable() throws MyDictionaryException {
        if(programController.getRepo().getPrgList().isEmpty())
            return;
        ...
        MyITable<Integer, Pair<Integer, ArrayList<Integer>>> sym = programController.getRepo().getById(programID).getSemaphoreTable();
        Enumeration<Integer> e = sym.keys();
        while(e.hasMoreElements())
        {
            Integer a = e.nextElement();
            SemaphoreTableView.getItems().add(new SemTableValue(a, sym.lookup(a).getKey(), sym.lookup(a).getValue()));
        }
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        ValueSemaphoreColumn.setCellValueFactory(new PropertyValueFactory<SemTableValue, Integer>("value"));
        IndexSemaphoreColumn.setCellValueFactory(new PropertyValueFactory<SemTableValue, Integer>("index"));
        ListSemaphoreColumn.setCellValueFactory(new PropertyValueFactory<SemTableValue, ArrayList<Integer>>("listValues"));
    ...
    public void loadProgram(IStmt statement, int index) throws MyException, IOException {
        ...
        MyITable<Integer, Pair<Integer, ArrayList<Integer>>> sem = new MyTable<>();
        ...
        PrgState program = new PrgState(executionStack, symbolTable, output, fileTable, heap1, sem);
