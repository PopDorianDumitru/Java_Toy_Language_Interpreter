...
public class MainProgramController implements Initializable {
    ...
    @FXML
    TableView<LockTableValue> LockTable;
    @FXML
    TableColumn<LockTableValue, Integer> LocationLockColumn;
    @FXML
    TableColumn<LockTableValue, Integer> ValueLockColumn;
    ...
    public void populateLockTable() throws MyDictionaryException {
        if(programController.getRepo().getPrgList().isEmpty())
            return;
        ...
        if(getIndex < 0)
            getIndex = 0;
        int programID = PrgStatesListView.getItems().get(getIndex);

        MyITable<Integer, Integer> sym = programController.getRepo().getById(programID).getLockTable();
        Enumeration<Integer> e = sym.keys();
        while(e.hasMoreElements())
        {
            Integer a = e.nextElement();
            LockTable.getItems().add(new LockTableValue(a, sym.lookup(a)));
        }
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        ValueLockColumn.setCellValueFactory(new PropertyValueFactory<>("value"));
        LocationLockColumn.setCellValueFactory(new PropertyValueFactory<>("location"));
    ...
    public void loadProgram(IStmt statement, int index) throws MyException, IOException {
        ...
        MyITable<Integer, Integer> LockTable = new MyTable<>();
        ...
        PrgState program = new PrgState(executionStack, symbolTable, output, fileTable, heap1, LockTable);
